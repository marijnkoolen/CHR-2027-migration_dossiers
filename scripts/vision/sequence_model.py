...
Design:
    ...
    2. A Transformer encoder (non-causal: at inference time the whole PDF is
       available, so every page can attend to every other page) turns the
       raw per-page embeddings into contextualized states.
    3. Start-page head: a page is a document boundary if its contextualized
    ...
    4. Segmentation: pages are grouped into runs between consecutive start
       ...
       predictions.
    5. Document-level heads: each segment's contextualized states are
...
class SequenceContextModel(nn.Module):
    ...
    def forward(
        ...
        prev = torch.zeros_like(context)
        prev[:, 1:] = context[:, :-1]
        boundary_feat = context - prev
        start_logits = self.start_head(torch.cat([context, boundary_feat], dim=-1)).squeeze(-1)
